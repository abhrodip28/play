...
public class DBConfig {
        ...
        try {
            if (datasource != null && destroyMethod != null && !destroyMethod.equals("")) {
                ...
                if (close != null) {
                    ...
                    datasource = null;
                    Logger.info("Datasource destroyed for db config " + dbConfigName);
                }
            }
        } catch (Throwable t) {
    ...
    private boolean changed(String propsPrefix) {
        ...
        if ("mem".equals(p.getProperty(propsPrefix))) {
            // If db.url or db.user or db.pass or db.driver is set but db=mem warn the user
            check(p, "memory", propsPrefix+".driver");
            check(p, "memory", propsPrefix+".url");
        ...
        if ("fs".equals(p.getProperty(propsPrefix))) {
            // If db.url or db.user or db.pass or db.driver is set but db=fs warn the user
            check(p, "fs", propsPrefix+".driver");
            check(p, "fs", propsPrefix+".url");
